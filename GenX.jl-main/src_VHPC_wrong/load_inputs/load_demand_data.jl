...
function load_demand_data!(setup::Dict, path::AbstractString, inputs::Dict)
    ...
    demand_in = get_demand_dataframe(TDR_directory)
    ...
    if setup["HeterogenousTimesteps"] == 0
        ...
        inputs["REP_PERIOD"] = convert(Int16, as_vector(:Rep_Periods)[1])
        inputs["H"] = convert(Int64, as_vector(:Timesteps_per_Rep_Period)[1])
        println("Debugging in Load_demand_data.jl line 129")
        println("Weights", inputs["Weights"])
        println("H", inputs["H"])
        println("omega", inputs["omega"] )
        println("my_dir", my_dir)

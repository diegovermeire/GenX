...
############ Needs to be changed ############
max_repetition = 1
unique_results_folder = "14_02_Greenfield_14eps_CO2=2_MC=0_UC=0"
results_folder_BF = "14_02_Borwnfield_14eps_CO2=2_MC=0_UC=0"

resource_folder= "resources"
policies_folder = "policies"
resource_folder_GF_enforced_BF = "resource_Greenfield_V1"
...
# #36 epsilons run
# epsilon_values = [0.001     , 0.10190909, 0.18263636, 
#        0.20281818, 0.21290909, 0.223     , 0.24318182,
#        0.25327273, 0.26336364, 0.29363636, 0.31381818, 
#        0.52572727, 0.73763636, 1.        ]
epsilon_values = [0.31381818]
            ...
            for CO2 in CO2Cap
                for repetition in 1:max_repetition
                    ...
                    try
                        ...
                        CSV.write(joinpath(directory,"mysetup.csv"), mysetup, bufsize=50_000_000)
                        CSV.write(joinpath(directory,"myinputs.csv"), myinputs, bufsize=50_000_000)
                    ...
                    res_folder = create_resource_policies_enforce_cap(dirname(dirname(@__FILE__)), outputs_path_read_BF, resource_folder_GF_enforced_BF)
                    ...
                    try
                        ...
                        new_row = DataFrame(epsilon=[eps], total_cost=[0], Run_time=[solve_time], 
                                            Time_Indexes=[myinputs["T"]], Build_model_time=[time_elapsed], write_outputs_time=[0], 
                                            termination_Status=[string(termination_status)], has_primal=[JuMP.has_values(EP)])
                        append!(accuracy_run_time, new_row)
                        # Convert rows into a DataFrame
                        data = DataFrame(accuracy_run_time)

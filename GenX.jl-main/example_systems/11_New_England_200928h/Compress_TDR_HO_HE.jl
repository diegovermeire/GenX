...
# epsilon_values = [0.98990909, 1.]
# TDR_HO_path = raw"C:\Users\Diego\GenX\GenX.jl-main\example_systems\11_New_England_200928h\TDR_results_HO_number_repr_period=2time_rep_period=8736"

function HO_TDR_to_HE_DBSCAN(epsilon_values, TDR_HO_path, path_to_save)
    # Define file names
    ...
    df_tot_eps = Dict{Any,DataFrame}()
    df_DBSCAN_stat_eps = Dict{Any,DataFrame}()
    ...
    for eps in epsilon_values
        ...
        df_tot_eps[eps] = deepcopy(df_tot)

        # Remove added columns from the original dataframe
        select!(df_tot, Not(["Cluster", "Rel_TimeStep"]))
        ...
        for label in unique(labels)
            if label != -1
                ...
                cluster_indices = findall(x -> x == label, df_tot_eps[eps][!, "Cluster"])
                # Sort the indices by "Time_Index" to ensure chronological order.
                sorted_indices = sort(cluster_indices, by = i -> df_tot_eps[eps][i, "Time_Index"])
                ...
                for i in 1:(n_sorted-1)
                    ...
                    if (df_tot_eps[eps][current_idx, "START_SUBPERIODS"] != 1) &&
                        (df_tot_eps[eps][current_idx, "Rep_Period"] == df_tot_eps[eps][next_idx, "Rep_Period"]) &&
                        (df_tot_eps[eps][current_idx, "Time_Index"] + 1 == df_tot_eps[eps][next_idx, "Time_Index"])
                        push!(index_consecutive_indices_to_merge, current_idx)
                        # Check if the next row is not consecutive, finalize the merge set
                        if ((i <= n_sorted - 2 && (df_tot_eps[eps][sorted_indices[i+1], "Time_Index"] + 1 != df_tot_eps[eps][sorted_indices[i+2], "Time_Index"])) ||
                            (i <= n_sorted - 2 && (df_tot_eps[eps][sorted_indices[i+1], "Rep_Period"] != df_tot_eps[eps][sorted_indices[i+2], "Rep_Period"])))
                            ...
                            for col in names(df_tot_eps[eps])
                                # Attempt to compute mean if the column element is Number
                                vals = df_tot_eps[eps][index_consecutive_indices_to_merge, col]
                                if all(x -> x isa Number, vals)
                                    mean_row[col] = mean(skipmissing(vals))
                                else
                            ...
                            mean_row["Rep_Period"] = df_tot_eps[eps][current_idx, "Rep_Period"]
                            ...
                            for col in keys(mean_row)
                                ...
                                if eltype(df_tot_eps[eps][!, col]) <: Integer
                                    df_tot_eps[eps][index_row_to_keep, col] = round(Int, mean_row[col])  # ✅ Ensure integer storage
                                else
                                    df_tot_eps[eps][index_row_to_keep, col] = mean_row[col]
                                end
                            end
        ...
        for idx in row_indexes_to_drop
            delete!(df_tot_eps[eps], idx)
        end

        # Reset "Time_Index" values sequentially
        df_tot_eps[eps] = copy(df_tot_eps[eps])  # This makes the row indices sequential 1:nrow
        df_tot_eps[eps][!, "Time_Index"] = collect(1:nrow(df_tot_eps[eps]))

        # Store statistics about clustering results
        qt_data_points = nrow(df_tot_eps[eps])
        max_rel_timestep = maximum(df_tot_eps[eps][!, "Rel_TimeStep"])
        df_DBSCAN_stat_eps[eps] = DataFrame(Qt_Data_Points = [qt_data_points], Max_Rel_TimeStep = [max_rel_timestep])
    end
    ...
    selected_demand_columns
    df_demand_core
    for eps in epsilon_values
        ## Step 1: Save modified data under the same format as the original df
        # Demand.csv
        df_demand_modified = deepcopy(dropmissing(df_tot_eps[eps][:, selected_demand_columns]))
        ...
        # Fuel.csv
        df_fuel_modified = deepcopy(df_tot_eps[eps][:, selected_fuel_columns])
        ...
        df_CF_eps = deepcopy(dropmissing(df_tot_eps[eps][:, selected_CF_columns]; disallowmissing=false))

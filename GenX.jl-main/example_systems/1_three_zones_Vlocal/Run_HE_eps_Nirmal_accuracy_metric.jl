...
############ Needs to be changed ############
max_repetition = 1
unique_results_folder_reading = "10_02_Run_HE_eps=0.025"
unique_results_folder_writing = "10_02_Brownfield_HE_eps=0.025"
base_folder = "resource_Greenfield_V1"
CO2Cap = [0]
UCommit = [0]
MinCapReq = [1]
        ...
        for commit in UCommit 
            for CO2 in CO2Cap
                for repetition in 1:max_repetition
                    ...
                    ########### Needs to be changed ############
                    run_results_folder = string(GenX.get_default_output_folder(dirname(@__FILE__)), "_", unique_results_folder_reading)
                    try
                    ...
                    outputs_path_read = joinpath(run_results_folder, string( string("_CO2=", CO2), string("_MinCapReq=",mincap), string("_COMMIT=", commit), string("_epsilon=", eps), string("_V",repetition)))
                    #############################################
                    ...
                    try
                        ...
                        ############ Needs to be changed ############
                        run_results_folder = string(GenX.get_default_output_folder(case), "_", unique_results_folder_writing)
                        try
                            if !isdir(run_results_folder)
                                mkpath(run_results_folder)
                            end

...
# unique_results_folder = "TDR_27_01_7.03pm"
...
# Debugging: Display the combinations
println(comb_df)

# Now replace the nested loops with a single loop
for row in eachrow(comb_df)
    time_rep_period = row.Timestep
    number_repr_period = row.NumberRepPeriod

# for time_rep_period in Timesteps_per_period
    println("time_rep_period", time_rep_period)
    println("number_repr_period", number_repr_period)

...
for i in 1:2
    ...
    # ################# 2. DEBUGGING on cost Fixed + Variable comparison #################
    # println("#############################################")
    ...
    #     gen = myinputs["RESOURCES"]
    #     p = myinputs["hours_per_subperiod"] #total number of hours per subperiod

    #     for t in t_test
    #         for y in THERM_COMMIT
    #             println("EP[:vP][y, t]", value(EP[:vP][y, t]))
    #             println("EP[:vP][y, t-1]", value(EP[:vP][y, GenX.hours_before_HE(t, 1, myinputs,p)]))
    #             println("regulation_term[y, t]" ,value(EP[:regulation_term][y, t]))
    #             println("reserves_term[y, t]" , value(EP[:reserves_term][y, t]))
    #             println("number of commited thermal generators at time t", value(EP[:vCOMMIT][y, t]))
    #             println( " <= k_ramp_up * cap_size * (vCommit - vStart)", GenX.heterogenous_ramp_small_variance(GenX.ramp_up_fraction(gen[y]),myinputs["Rel_TimeStep"][t])  * GenX.cap_size(gen[y]) *(value(EP[:vCOMMIT][y, t]) - value(EP[:vSTART][y, t])))
    #             println(" second term behind ramp constraint", min(myinputs["pP_Max"][y, t], max(GenX.min_power(gen[y]), GenX.ramp_up_fraction(gen[y]))) * GenX.cap_size(gen[y]) * value(EP[:vSTART][y, t]))
    #             println("min_power(gen[y]) * cap_size(gen[y]) * EP[:vSHUT][y, t]", GenX.min_power(gen[y]) * GenX.cap_size(gen[y]) * value(EP[:vSHUT][y, t]))
    #             println("               ")
    #         end
    #     end
    ...
    ############################### DEBUGGING thermal_non_commit max Power and power output increase ################################
#     gen = myinputs["RESOURCES"]
#     T = myinputs["T"]     # Number of time steps (hours)
#     Z = myinputs["Z"]     # Number of zones
#     p = myinputs["hours_per_subperiod"] #total number of hours per subperiod
#     THERM_NO_COMMIT = myinputs["THERM_NO_COMMIT"]
#     if i == 1
#         t_test = [99, 100]
#     elseif i==2
#         t_test = [78]
...
#     for y in THERM_NO_COMMIT
#         for t in t_test
#             println("Max thermal power output increase between time t $t and t-1 ",  GenX.heterogenous_ramp_small_variance(GenX.ramp_up_fraction(gen[y]),myinputs["Rel_TimeStep"][t]) * value(EP[:eTotalCap][y]))
#             println("Maximum power generated per technology y $y at hour t $t  ", myinputs["pP_Max"][y, t] * value(EP[:eTotalCap][y]))
#             println("myinputs[pP_Max][y, t]", myinputs["pP_Max"][y, t])
#             println("EP[:eTotalCap][y])", value(EP[:eTotalCap][y]))
#         end
#     end
    ###################################################################################################################################
end

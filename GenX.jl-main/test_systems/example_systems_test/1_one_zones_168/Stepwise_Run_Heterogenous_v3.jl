...
for i in 1:2
    ...
    ################# DEBUGGING thermal activation of generators #################
    THERM_COMMIT = myinputs["THERM_COMMIT"]
    gen = myinputs["RESOURCES"]
    G = myinputs["G"]     # Number of resources (generators, storage, DR, and DERs)
    Up_Time = zeros(Int, G)
    Up_Time[THERM_COMMIT] .= Int.(floor.(GenX.up_time.(gen[THERM_COMMIT])))
    p = myinputs["hours_per_subperiod"] #total number of hours per subperiod

    if i==1 
        t_test = [65, 66, 67, 68, 69, 70, 71, 72]
    else
        t_test = [55, 56, 57, 58]
    end
    ...
    for y in THERM_COMMIT
        for t in t_test
            println("Capacity size of generator $y :", GenX.cap_size(gen[y]))
            println("vCOMMIT of gnerator $y at time $t :", value(EP[:vCOMMIT][y, t]))
            println("vSTART af generator $y at time $t :", value(EP[:vSTART][y, t]))
            println("vCOMMIT - vSTART ", value(EP[:vSTART][y, t]) - value(EP[:vCOMMIT][y, t]) )
            println("Minimum up time, states that vCOMMIT of generator $y at time $t must be greater than ", sum(value(EP[:vSTART][y, u]) for u in (GenX.iterate_backward(myinputs, t, Up_Time[y]))))
            println("GenX.iterate_backward(myinputs, t, Up_Time[y]):t))", unique(GenX.iterate_backward(myinputs, t, Up_Time[y])))
        end
    end

    # for current_t in t_test
    #     # Array containing all time_indexes up_down_tao from 
    #     time_set = []
    #     p = myinputs["hours_per_subperiod"] #total number of hours per subperiod
    #     push!(time_set, current_t)
    #     for t_back in 1:(Up_Time[1]-1)
    #         push!(time_set, GenX.hours_before_HE(current_t, t_back, myinputs, p))
    #     end
    #     println(time_set)
    # end
    # ###################################################################################
    # GenX.hours_before_HE(55, 6, myinputs, p)
end

...
function storage_all!(EP::Model, inputs::Dict, setup::Dict)
    ...
    elseif (setup["HeterogenousTimesteps"] == 1)
        ...
        cSoCBalStart[t in START_SUBPERIODS, y in CONSTRAINTSET],
        EP[:vS][y,t]== 
        (((1 - self_discharge(gen[y]))^inputs["Rel_TimeStep"][t]) * (EP[:vS][y, T] - sum(EP[:vP][y, t] * (1 / efficiency_down(gen[y])) / ((1-self_discharge(gen[y]))^i) for i in 0:(inputs["Rel_TimeStep"][t]-1) )) + 
        ...
        #     +
        #     (eff iciency_up(gen[y]) * EP[:vCHARGE][y, t]* inputs["Rel_TimeStep"][t]) -

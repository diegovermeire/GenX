...
function thermal_no_commit!(EP::Model, inputs::Dict, setup::Dict)
    ...
    elseif ((setup["HeterogenousTimesteps"] == 1) && (setup["TimeDomainReduction"] == 0))
        @constraints(EP,
            begin
                ## Maximum ramp up between consecutive hours
                [y in THERM_NO_COMMIT],
                EP[:vP][y, 1] - EP[:vP][y, T] <= heterogenous_ramp_small_variance(ramp_up_fraction(gen[y]), inputs["Rel_TimeStep"][1], inputs["Rel_TimeStep"][T]) * EP[:eTotalCap][y]
                
                [y in THERM_NO_COMMIT],
                EP[:vP][y, T] - EP[:vP][y, 1] <= heterogenous_ramp_small_variance(ramp_down_fraction(gen[y]), inputs["Rel_TimeStep"][T], inputs["Rel_TimeStep"][1]) * EP[:eTotalCap][y]

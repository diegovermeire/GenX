...
function thermal_commit!(EP::Model, inputs::Dict, setup::Dict)
    ...
    elseif ((setup["HeterogenousTimesteps"] == 1) && (setup["TimeDomainReduction"] == 0))
        ...
        @constraints(EP,
            begin
                [y in THERM_COMMIT],
                EP[:vCOMMIT][y, 1] ==
                EP[:vCOMMIT][y, T] + EP[:vSTART][y, 1] - EP[:vSHUT][y, 1]
    ...
    # rampup constraints
    #       - min_power: minimum generation level for a unit as a fraction of total capacity
    if setup["HeterogenousTimesteps"] == 0
    ...
    elseif ((setup["HeterogenousTimesteps"] == 1) && (setup["TimeDomainReduction"] == 0))
        @constraint(EP, [y in THERM_COMMIT],
            ...
            reserves_term[y, 1]<= heterogenous_ramp_small_variance(ramp_up_fraction(gen[y]),inputs["Rel_TimeStep"][1], inputs["Rel_TimeStep"][T])  * cap_size(gen[y]) *
                                (EP[:vCOMMIT][y, 1] - EP[:vSTART][y, 1])
                                +
                                min(inputs["pP_Max"][y, 1],
                                    max(min_power(gen[y]), ramp_up_fraction(gen[y]))) *
                                cap_size(gen[y]) * EP[:vSTART][y, 1]
                                -
                                min_power(gen[y]) * cap_size(gen[y]) * EP[:vSHUT][y, 1])

        @constraint(EP, [y in THERM_COMMIT, t in 2:T],
            EP[:vP][y, t] - EP[:vP][y, t-1] + regulation_term[y, t] +
            reserves_term[y, t]<= heterogenous_ramp_small_variance(ramp_up_fraction(gen[y]),inputs["Rel_TimeStep"][t], inputs["Rel_TimeStep"][t-1])*cap_size(gen[y])*
                                (EP[:vCOMMIT][y, t] - EP[:vSTART][y, t])
                                +
                                min(inputs["pP_Max"][y, t],
                                    max(min_power(gen[y]), ramp_up_fraction(gen[y]))) *
    ...
    elseif ((setup["HeterogenousTimesteps"] == 1) && (setup["TimeDomainReduction"] == 0))
        ...
        reserves_term[y, T]<= heterogenous_ramp_small_variance(ramp_down_fraction(gen[y]),inputs["Rel_TimeStep"][T], inputs["Rel_TimeStep"][1]) * cap_size(gen[y]) *
                                (EP[:vCOMMIT][y, 1] - EP[:vSTART][y, 1])
                                -
                                min_power(gen[y]) * cap_size(gen[y]) * EP[:vSTART][y, 1]
                                +
                                min(inputs["pP_Max"][y, 1],
                                    max(min_power(gen[y]), ramp_down_fraction(gen[y]))) *
                                cap_size(gen[y]) * EP[:vSHUT][y, 1])
    ...
    elseif ((setup["HeterogenousTimesteps"] == 1) && (setup["TimeDomainReduction"] == 0))
        ...
        @constraint(EP, [y in THERM_COMMIT, t in 1:T],
            EP[:vCOMMIT][y,
                t]>=sum(EP[:vSTART][y, u] for u in (iterate_backward(inputs, t, Up_Time[y]))))
